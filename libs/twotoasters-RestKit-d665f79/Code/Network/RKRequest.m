...
- (void)prepareURLRequest {
	...
	// NOTE: Using the synchronous call for now, which cannot refresh the access token!
	if (_auth != nil) {
		BOOL authorized = [_auth authorizeRequest:_URLRequest];
		if (!authorized) {
			NSLog(@"Unable to successfully authorize request, expect it to fail. Should use asynchronous calls to avoid this issue.");
		}
	}
}
...
- (void)fireAsynchronousRequest {
	...
	// CS FIX
    RKResponse* response = [[(RKResponse*)[RKResponse alloc] initWithRequest:self] autorelease];
