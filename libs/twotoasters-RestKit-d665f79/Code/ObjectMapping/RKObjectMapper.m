...
- (void)setPropertiesOfModel:(NSObject<RKObjectMappable>*)model fromElements:(NSDictionary*)elements {
    ...
    if (anyValuesSet == NO) {
        NSLog(@"WARNING: RestKit did not find any mappable values for the specified object %@. If you have specified an explicit objectClass for mapping, be sure that the keyPath has been set to target your mappable data.", model);
    }
}
